function [ ] = savePogoInp( fileName, model, fileVer )


%savePogoInp - save a pogo input file
%   savePogoInp( fileName, model, ver )
%
% fileName - the filename to save to
% ver - what version of the file to save to, 1.07, 1.08, 1.09 and 1.10 supported [optional]
...
% model.matTypeRefs - which of the material types each element refes to, length nEls
% model.orientRefs - which of the orientations each element refes to, length nEls
% 
...
% 
% model.frames{n} - structure for each frame 
%  several frames can be used in each model to specify different source
%  setups; can be useful for array imaging
% model.frames{n}.ntSig - number of time points for all the signals (commonly just set to model.nt)
% model.frames{n}.dtSig - time step for the signals (commonly just set to model.dt)
% 
% model.frames{n}.sigs{m} - structure for each of the signals used
% 
% model.frames{n}.sigs{m}.isDofGroup - 0 if not, 1 if dofGroups are referred to in dofSpec (NB only from v1.08)
% if 0:
% model.frames{n}.sigs{m}.nodeSpec - nodes the signal is applied to. 
...
%   DOF in the model.
% model.frames{n}.sigs{m}.dofSpec - DOF to apply the signal to (matches nodeSpec) in (range: 1 to model.nDofPerNode)
% if 1:
% model.frames{n}.sigs{m}.dofGroup - dof groups, defined earlier
%
% model.frames{n}.sigs{m}.sigType - 0 force, 1 displacement or (unused at present) 2 velocity
% model.frames{n}.sigs{m}.sigAmps - amplitudes the signals are multiplied by for each dof specified
% model.frames{n}.sigs{m}.sig - signal (length model.ntSig)
% 
...
% 
% model.fieldStoreIncs - which increments to output the field at
...
%     header(1:13) = '%pogo-inp1.11';
    ...
    error('Only v1.07, v1.08, v1.09 and v1.10 supported, not %f', fileVer);
end
header(14) = 0;
...
runNameWrite(1:length(model.runName)) = model.runName;
runNameWrite(length(model.runName)+1) = 0;
...
if ~isfield(model,'elTypeRefs')
    error('model.elTypeRefs field must be set')
end
    ...
    for sCnt = 1:nSigs
        ...
        if isDofGroup
            if ~isfield(s,'dofGroup') 
                error('model.frames{%d}.sigs{%d}.dofGroup is not defined',fCnt,sCnt)
            end
