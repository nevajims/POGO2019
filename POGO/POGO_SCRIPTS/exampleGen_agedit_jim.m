% AG 10/05/2018
% Split processing into 2 part to help manual meshing on linux
clear

MESH = 0;
JIM = 1;

PATH = 'C:\Users\jim\Desktop\New folder (2)';
OUTPUT = 'example_jim3.pogo-inp';
...
if JIM == 1
    JIM_DATA_PATH = '\\nas\Public\POGO\JIM\meshes\MESH_RAIL_56_1.mat';
    jmesh = load(JIM_DATA_PATH);
        
    m = convertJMesh2PogoMesh( jmesh );
    
    mesh_details = get_element_details(jmesh.mesh);
    ...
    %no absorbing boundary on the bottom edge

    %no decoupling by duplicate

    %save the file - this is done in the slightly older v1.07 format
    %savePogoInp('example.pogo-inp',m3,1.07);
    
    savePogoInp(strcat(PATH, OUTPUT),m3);
