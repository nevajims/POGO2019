...
% create an app from
% old
 function  [Pogo_model_input]     =   extract_excitation(N_cycles , points_per_trace , file_name , window_type , mode_no , chosen_freq_kHz , node_plot_index, doplot )
...
chosen_freq                                  = chosen_freq_kHz*1000                       ; 
...
freq_index = max(find((reshaped_proc_data.freq(:,4)-chosen_freq)<=0)); 
...
sig_x = sig_x / max_all_sig  ;
sig_y = sig_y / max_all_sig  ;
sig_z = sig_z / max_all_sig  ;
...
Pogo_model_input.stats.excit_freq_MHz  = chosen_freq/1000                 ;
...
plot(time_,sig_x(:,node_plot_index)   ,'.-' )
ylim([-1 1])
ylabel('ampl ')
set(gca,'xtick',[])
title(['X: A = ', num2str(100*max(sig_x(:,node_plot_index))),' %  ,P = ',num2str(round(180*phase_x/pi)),' deg' ])

subplot(3,2,3)
plot(time_,sig_y(:,node_plot_index)   ,'.-' )
ylim([-1 1])
ylabel('ampl ')
set(gca,'xtick',[])
title(['Y: A = ', num2str(100*max(sig_y(:,node_plot_index))),' % ,P = ',num2str(round(180*phase_y/pi)),' deg' ])

subplot(3,2,5)
plot(time_,sig_z(:,node_plot_index) ,'.-' )
ylim([-1 1])
xlabel ('time (us)')
ylabel('ampl ')
title(['Z: A = ', num2str(100*max(sig_z(:,node_plot_index))),' % ,P = ',num2str(round(180*phase_z/pi)),' deg' ])

suptitle(['(Mode = ',num2str(mode_no),', Freq = ' ,num2str(chosen_freq/1000),'kHz) Node = ',num2str(node_plot_index),' /',num2str(size(reshaped_proc_data.ms_x,1)),', win = ',wind_name   ,', (',file_name_removed ,').'])
disp(['Mode = ',num2str(mode_no),', Freq = ' ,num2str(chosen_freq/1000), ' kHz (',num2str(N_cycles),' cycles ',wind_name,').'])
...
end %if  chosen_freq > min (reshaped_proc_data.freq(:,mode_no)) && chosen_freq_kHz <  max (reshaped_proc_data.freq(:,mode_no))
